...
namespace Server.Infrastructure.Validator
{
                                                                   ...
                                                                   where TProperty : notnull
    {
        ...
        // Validates the phone entered using regex to
        // ensure it matches the format of a phone.
        public ValidatorInputFieldStruct<TModel, TProperty> ValidatePhone(string patternCheckPhone = "^(0[2-9]{1})-?[0-9]{7}$", string errorMessage = "The phone is invalid")
        {
            validations.Add(new ValidationStruct<TProperty>(
                value =>
                         value == null ||
                        string.IsNullOrEmpty(value.ToString()) ||
                        Regex.IsMatch(value.ToString()!, patternCheckPhone),
                errorMessage
            ));
            return this;
        }
        ...
        // Extension method to validate collections
        public ValidatorInputFieldStruct<TModel, TProperty> ValidateCollection<TItem>(
            Action<ValidatorStruct<TItem>> collectionValidation,
            string collectionErrorMessage = "Collection validation failed")
            where TItem : class
        {
            validations.Add(new ValidationStruct<TProperty>(
                value =>
                {
                    ...
                    if (value is not IEnumerable<TItem> collection) return false;

                    var itemValidator = new ValidatorStruct<TItem>();
                    collectionValidation(itemValidator);
        ...
        // Method to validate minimum collection size
        public ValidatorInputFieldStruct<TModel, TProperty> MinCollectionSize(
            int minSize,
            string? errorMessage = null)
        {
            validations.Add(new ValidationStruct<TProperty>(
                ...
                {
                    if (value == null) return true;
                    if (value is not IEnumerable<object> collection) return false;
                    return collection.Count() >= minSize;
                },
                errorMessage ?? $"Collection must have at least {minSize} items"
            ));
            return this;
        }

        // Method to validate maximum collection size
        public ValidatorInputFieldStruct<TModel, TProperty> MaxCollectionSize(
            int maxSize,
            string? errorMessage = null)
        {
            validations.Add(new ValidationStruct<TProperty>(
                ...
                errorMessage ?? $"Collection must have at most {maxSize} items"
            ));
            return this;
        }
    }
